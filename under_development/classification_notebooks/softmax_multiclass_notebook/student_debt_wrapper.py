...
def plot_pts(x,y,ax,xlabel,ylabel):
    ...
    ax.set_ylim(0.9*np.min(y),1.2*np.max(y))
    s = np.linspace(np.min(x),np.max(x) + (np.max(x) - np.min(x))/float(6), 7)
    ax.set_xticks(s)
    ax.set_xticklabels([2004,2006,2008,2010,2012,2014,2015])
    
# plot fit
def plot_fit(x,y,w,ax):
    # generate area over which to make line
    s = np.linspace(np.min(x),np.max(x) + (np.max(x) - np.min(x))/float(5), 100)
    s.shape = (100,1)
...
def make_cost_surface(x,y,ax):
    ...
    ax.set_yticks([-1,0,1])
    ax.set_zticks([0,300])
    ax.set_ylabel('b - intercept parameter')
    ax.set_xlabel('w - slope parameter')
    ax.set_zlabel('g(b,w)',rotation = 90)
